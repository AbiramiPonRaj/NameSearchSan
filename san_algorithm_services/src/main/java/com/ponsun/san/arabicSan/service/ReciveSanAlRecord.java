...
import com.ponsun.san.algorithm.service.ArabicBatchProcessor;
...
public class ReciveSanAlRecord {
    private final ArabicBatchProcessor arabicBatchProcessor;
    ...
    @SneakyThrows
    public List<ResponseData> uicalculateScoreSinglePara(MultiParaSearchData multiParaSearchData) {
        List<ResponseData> uirecordDTOS = new CopyOnWriteArrayList<>();
    ...
    private void processChunks(MultiParaSearchData multiParaSearchData, List<List<ArabicSanData>> arabicSanchunkedData, String filter1,
                               ...
                               List<ResponseData> uirecordDTOS) {
        ...
        List<CompletableFuture<Void>> futures = arabicSanchunkedData.stream()
                .map(chunk -> CompletableFuture.runAsync(() -> {
                    List<ResponseData> chunkResult = null;
                    switch (entityType) {
                        case 1://
                            chunkResult = arabicBatchProcessor.processChunksInParallelWithFilter(chunk, multiParaSearchData, filter1, token1, mapList1,"",temp);
                            break;
                        case 2:
                            chunkResult = arabicBatchProcessor.processEntityChunksInParallelWithFilter(finalSuffixes, finalName1_withoutsuf, finalToken1_withoutsuf,chunk, multiParaSearchData,filter1, token1);
                            break;
                        case 3:
                            chunkResult = arabicBatchProcessor.processChunksInParallelWithFilter(chunk,multiParaSearchData, filter1, token1, mapList1, finalName1_withoutsuf, finalToken1_withoutsuf);
                            break;
                        case 4:
                            chunkResult = arabicBatchProcessor.processChunksInParallelWithFilter(chunk, multiParaSearchData,filter1, token1,mapList1,"",temp);
                            break;
...
//                // System.out.println("✅ Processing batch size: " + batchData.size() + " | Total records read: " + totalRecordsRead.get());
    ...
    private static final String jsonFileName = "ProcessedData1Ldata.json";
