...
@Service
public class ArabicBatchProcessor {
    ...
    public static List<ResponseData> processChunksInParallelWithFilter(
        ...
        List<ResponseData> uirecordDTOS = chunk.stream()
    ...
    public static List<ResponseData> processEntityChunksInParallelWithFilter(LinkedHashSet<String> suffixes,
    ...
    public static List<ResponseData> processEntityRemitChunksInParallelWithFilter(
            List<ArabicSanData> chunk, MultiParaSearchData multiParaSearchData,String filter1,
            List<String> token1) {
    ...
    private static boolean isValidScore(ResponseData dto, MultiParaSearchData multiParaSearchData) {
        Integer score = dto.getMaxWeightageScore();
        return score != null && score >= multiParaSearchData.getScore();
    }

    private static List<ResponseData> processSingleRecord(ArabicSanData batchData, MultiParaSearchData multiParaSearchData,String filter1,List<String> token1,List<Map<String, List<String>>> mapdate1List,String  name1_withoutsuf,List<String> token1_withoutsuf) {
        try {
            ...
            Integer maxScore = (int) Arrays.stream(scores, 1, scores.length).parallel().max().orElse(0);

            // Return the UiReciveSingleRecordDto object with the calculated scores
//            Integer sessionID, String UID, String sourceName, Integer maxWeightageScore, Integer algorithm1WeightageScore, Integer algorithm2WeightageScore, Integer algorithm3WeightageScore, Integer algorithm4WeightageScore, Integer algorithm5WeightageScore, String matchedName
            return List.of(new ResponseData(1,personId+"","personId", maxScore, (int) scores[1],(int) scores[2],(int) scores[3],(int) scores[4],(int) scores[5], batchData.getName()));
        } catch (Exception e) {
    ...
    private static List<ResponseData> processEntitySingleRecord(ArabicSanData batchData, MultiParaSearchData multiParaSearchData,String filter1,List<String> token1,String name1_withoutsuf,List<String> token1_withoutsuf ) {
        try {
            String firstName = multiParaSearchData.getName() != null ? multiParaSearchData.getName() : "";
            String id1       = multiParaSearchData.getId() != null ? multiParaSearchData.getId() : "";
            String country1  = multiParaSearchData.getCountry() != null ? multiParaSearchData.getCountry() : "";
...
//            double maxScore = Arrays.stream(scores, 1, scores.length).parallel().max().orElse(0);
            // Return the UiReciveSingleRecordDto object with the calculated scores
            return List.of(new ResponseData(1,"","personId", maxScore, (int) scores[0],(int) scores[0],(int) scores[0],(int) scores[0],(int) scores[0], batchData.getName()));
        } catch (Exception e) {
            //Logger.error("Exception processing data for name: " + batchData.getName(), e);
            // System.out.println("Exception processing data for name: " + batchData.getName() +":"+ e);
            return Collections.emptyList();  // Return empty list in case of failure
    ...
    private static List<ResponseData> processEntitySingleRecord(ArabicSanData batchData, MultiParaSearchData multiParaSearchData,String filter1,List<String> token1) {
        try {
            String firstName = multiParaSearchData.getName() != null ? multiParaSearchData.getName() : "";
            String id1       = multiParaSearchData.getId() != null ? multiParaSearchData.getId() : "";
            String country1  = multiParaSearchData.getCountry() != null ? multiParaSearchData.getCountry() : "";
...
//            double maxScoreReturn = Arrays.stream(scores, 1, scores.length).parallel().max().orElse(0);
        }
        else {
            Arrays.fill(scores, 1, scores.length, 0);
        }
...
//                .orElse(Double.NaN); // Return Double.NaN if the array is empty
    ...
    private static void applyCountryAndDobWeights(double[] computedScores, String country1, String country2, String dob1, String dob2,List<Map<String, List<String>>> mapdate1List) {
        ...
        IntStream.range(1, computedScores.length).parallel().forEach(i ->
    ...
    private static double safeCalculate(ArabicBatchProcessor.Supplier<Double> calculation, String type, String queryCustomer, String str2) {
        try {
